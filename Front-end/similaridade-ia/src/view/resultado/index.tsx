...
export function Resultado() {
    ...
    useEffect(() => {
        api.get(`/cases${localStorage.getItem('Caso')}`).then(response => {
            setResultado(response.data.desc_doenca);
        })
    }, [])
    ...
    return (
        <section >
            ...
            <div className="w-full flex items-center justify-end">
                <button className="bg-gray-300 duration-500 h-10 w-20 rounded mt-2">Finalizar</button>
            </div>
        </section>
    )
}